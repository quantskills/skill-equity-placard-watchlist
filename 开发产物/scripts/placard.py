...
术语对照（交付语言规范：专业术语必配简明解释）：

| 术语 | 含义 |
|---|---|
| 举牌 | 单一股东持股达到 5%，触发《证券法》强制披露 |
| 举牌梯度 | 5% 之后每增减 5% 需再次披露；档位越高越接近谋求控制权 |
| 权益变动 | 股东持有的公司股份数量或比例发生变化 |
| 一致行动人 | 约定共同行使表决权的多方，法律上合并计算持股（本数据源无此字段，见已知限制） |
| 短线交易归入权 | 举牌后 6 个月内反向交易的收益须归公司，短期内难以套现 |
| 财务投资 vs 战略投资 | 赚取价差 vs 谋求董事席位乃至控制权 |
...
GLOSSARY: dict[str, str] = {
    ...
    "锁定期": "举牌后 6 个月内反向交易的收益须归公司，短期内难以套现",
    "财务投资": "以赚取价差为目的，不谋求经营参与",
    "战略投资": "谋求董事席位乃至控制权，倾向持续增持",
    "通道账户": "北向托管、回购专户等代持类账户，持股越过 5% 不构成举牌",
    "增持强度": "本期相对上期增持的百分点数",
    "PIT": "只用当时已公告的信息下判断，不偷看后来才披露的数据",
}
...
def crossed_lines(prev: float, cur: float, lines: list) -> list:
    ...
    例：上期 4.6%、本期 11%，则一次越过 5% 与 10% 两条线。
    """
...
def infer_share_change(panel: pd.DataFrame, config: Optional[dict] = None) -> pd.DataFrame:
    ...
    即：公司增发新股后总股本增加，未交易股东的持股比例同步下降，这不是减持。真机实测 301629.SZ 有 7 个股东比值全是 0.75000（发行 1/3 新股），
...
def detect_events(panel: pd.DataFrame, config: Optional[dict] = None) -> pd.DataFrame:
    ...
    for _, r in d.iterrows():
        ...
        rows.append({
            ...
            # 举牌轮次：跨过第几档（5%=1、10%=2 …），档位越高越接近控制权诉求
...
def _lock_until(announce_date: Any, months: int) -> Optional[str]:
    ...
    该日期之前反向交易的收益须归公司所有（《证券法》第 44 条），故短期内难以套现。
    """
...
def plain_text(ev: dict) -> str:
    ...
    if lock and et in ("placard", "placard_raise"):
        bits.append(f"短线交易归入权锁定至 {lock}：该日前反向交易收益须归公司，期间难以套现。")
    return "".join(bits)
